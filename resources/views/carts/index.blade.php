                        ...
                        <div class="bg-white shadow-md rounded-lg p-4">
                            ...
                            <p class="text-sm text-gray-500">Total Price:
                                ...
                                <form action="{{ route('cart.add', $product->id) }}" method="POST">
                                    @csrf
                                    <p style="background: linghtgray; padding: 5px; border-radius: 5px; cursor: pointer; left: 0; position: relative; bottom: 0; margin: 0;"  onclick="this.parentElement.submit()"><i class='bx bx-plus' ></i></p>
                                </form>
                                <form action="{{ route('cart.minus', $product->id) }}" method="POST">
                                    @csrf
                                    <p style="background: linghtgray; padding: 5px; border-radius: 5px; cursor: pointer; left: 0; position: relative; bottom: 0; margin: 0;"  onclick="this.parentElement.submit()"><i class='bx bx-minus' ></i></p>
                                </form>
